...
/**
 * Theme Trainterra - course page layout.
 *
 * COPIE DE theme/boost_union/layout/drawers.php
 *
 ...
 * 
 * Modifications Trainterra apportées : on a copié le layout drawers.php de boost_union puis on passe l'id du cours au template (+ fullname + shortname + imageurl) pour pouvoir afficher un bouton avec l'url de la page d'accueil du cours
 * J'ai été obligé d'importer entièrement le dossier layout/includes de Boost_union pour garder la compatibilité des fonctionnalités
 *
 *
 * @package   theme_theme_sos
 * @copyright Damien Will, trainterra.fr
...
// Récupération de l'ID + fullname + shortname + url + imageurl du cours à passer au template pour afficher un bouton de retour à la page du cours dans le course index
$courseid = $COURSE->id;
$coursename= $COURSE->fullname;
$courseshortname= $COURSE->shortname;
$courseurl = new moodle_url('/course/view.php', ['id' => $courseid]);
$courseimagefile = course_get_courseimage($COURSE);
// Vérification si l'image existe
if ($courseimagefile) {
    // Construction de l'URL correcte de l'image
    $courseimageurl = moodle_url::make_pluginfile_url(
        $courseimagefile->get_contextid(),
        $courseimagefile->get_component(),
        $courseimagefile->get_filearea(),
        null, // $courseimagefile->get_itemid() je le passe à null sinon il s'insère dans l'url alors que je n'en n'ais pas besoin !
        $courseimagefile->get_filepath(),
        $courseimagefile->get_filename(),
        false
    ); 
}

// Je génère le html du bouton de retour au cours via la fonction du renderable backtocoursebutton.php
// Je passe l'ID du cours, le nom du cours, l'url de la page d'accueil du cours et l'url de l'image du cours
$backtocoursebutton = new \theme_theme_sos\output\backtocoursebutton(
    $coursename,
    $courseurl,
    $courseimageurl
);

// On génère le HTML du bouton grace au renderer.php de notre theme
$renderer = $PAGE->get_renderer('theme_theme_sos'); // on récupère le renderer du thème
$backtocoursebuttonhtml = $renderer->render_backtocoursebutton($backtocoursebutton); // on génère le HTML du bouton grace à la fonction render_backtocoursebutton de notre renderer
...
$templatecontext = [
    ...
    'backtocoursebutton' => $backtocoursebuttonhtml // nouveau bouton généré
];
...
echo $OUTPUT->render_from_template('theme_theme_sos/custom_course/course', $templatecontext);

...
/**
 * Theme theme_sos - Theme config
 *
 * @package    theme_theme_sos
 * @copyright  2024 Damien Will <damien.will@trainterra.fr>
 *             based on code developped by 2024 Alexander Bias <bias@alexanderbias.de>
...
// This move will save us from duplicating all lines from Boost Union's config.php into theme_sos's config.php.
...
// Then, we require theme_sos's locallib.php to make sure that it's always loaded.
require_once($CFG->dirroot . '/theme/theme_sos/locallib.php');

// Next, we overwrite only the settings which differ between Boost Union and theme_sos.
$THEME->name = 'theme_sos';
$THEME->scss = function($theme) {
    return theme_theme_sos_get_main_scss_content($theme);
};
$THEME->parents = ['boost_union', 'boost'];
$THEME->extrascsscallback = 'theme_theme_sos_get_extra_scss';
$THEME->prescsscallback = 'theme_theme_sos_get_pre_scss';
...
$THEME->layouts = [
    ...
    'frontpage' => [
        ...
        // Main course page. On récupère le layout course.php de notre thème qui est customisé.
    'course' => [
        'file' => 'course.php',
...
// Lastly, we replicate some settings from Boost Union at runtime into theme_sos's settings.
...
// it would be quite ugly to duplicate all of these settings again to theme_sos.

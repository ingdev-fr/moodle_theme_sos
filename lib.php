 ...
 * @return string
 */
function theme_theme_sos_get_main_scss_content($theme) {
    global $CFG;
    ...
    // This way, theme_sos will ship the same SCSS code as Boost Union itself.
    $scss = theme_boost_union_get_main_scss_content(\core\output\theme_config::load('boost_union'));

    // And add theme_sos's main SCSS file to the stack.
    $scss .= file_get_contents($CFG->dirroot . '/theme/theme_sos/scss/post.scss');
 ...
 * @return string
 */
function theme_theme_sos_get_pre_scss($theme) {
    global $CFG;
    ...
    // If such a fault happens, the theme_sos administrator can switch the inheritance to 'Duplicate'.
    // This way, we will add the pre SCSS code with the explicit use of the Boost Union configuration to the stack.
    $inheritanceconfig = get_config('theme_theme_sos', 'prescssinheritance');
    ...
    // And add theme_sos's pre SCSS file to the stack.
    $scss .= file_get_contents($CFG->dirroot . '/theme/theme_sos/scss/pre.scss');
 ...
 * @return string
 */
function theme_theme_sos_get_extra_scss($theme) {
    global $CFG;
    ...
    $inheritanceconfig = get_config('theme_theme_sos', 'extrascssinheritance');
    if ($inheritanceconfig == THEME_BOOST_UNION_CHILD_SETTING_INHERITANCE_DUPLICATE) {
...
/**
 * Callback function for theme_boost_union to allow theme_sos to add cards to the Boost Union settings overview page.
 * This function is expected to return an array of arrays containing values with the keys 'label', 'desc', 'btn' and 'url'.
 *
 * @return array
 */
function theme_theme_sos_extend_busettingsoverview() {

    $cards[] = [
        'label' => get_string('pluginname', 'theme_theme_sos'),
        'desc' => get_string('settingsoverview_buc_desc', 'theme_theme_sos'),
        'btn' => 'primary',
        'url' => new \core\url('/admin/settings.php', ['section' => 'theme_theme_sos']),
    ];
